    ...
    for url in urls:
        ...
        if flag == "tiki":
            ...
            try:
                ...
                if data and 'responseBody' in data:
                    response_body = data['responseBody']
                    # Lazada response may contain item_id
                    item_id = response_body.get('item_id', None)
                    if not item_id:
                        # Fallback to extracting item_id from URL
                        item_id_match = re.search(r'-i(\d+)-s(\d+)\.html', url)
                    ...
                    logging.warning(f"No data returned for Lazada URL: {url}")
            except Exception as e:
